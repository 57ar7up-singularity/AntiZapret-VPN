...
Через [полный VPN](https://www.kaspersky.ru/resource-center/definitions/what-is-a-vpn) (файлы vpn-\*) работают все сайты, доступные с вашего сервера, что позволяет обходить все ограничения

**Внимание!**\
Для правильной работы AntiZapret VPN и полного VPN нужно:
1. Отключить безопасный/частный/персональный DNS в [браузерах](https://ya.ru/search/?text=отключить+DNS+в+браузере&promo=force_neuro) и на [устройствах](https://ya.ru/search/?text=отключить+DNS+на+устройстве&promo=force_neuro)
...
```
4. Отключить IPv6 на устройстве, в подключении на роутере, в локальной сети 
...
### 7. Проверить, идет ли домен через АнтиЗапрет
```
/root/antizapret/check-host.sh <домен_или_ссылка> [домен_или_ссылка]...
```
Например
```
/root/antizapret/check-host.sh https://www.cybenetics.com/ rutracker.org
```
Скрипт покажет для каждого домена: попал ли он в список АнтиЗапрета, что отдает клиенту DNS АнтиЗапрета, какой у домена реальный IP-адрес и уходит ли трафик в туннель или напрямую через провайдера клиента\
Это самый быстрый способ понять, почему сайт не открывается или открывается наполовину, подробнее в [FAQ](#8-почему-сайт-не-открывается-или-открывается-наполовину)
...
- [aeza.net](https://aeza.net/?ref=529527) - крипта, юрлицо не в РФ - ссылка для регистрации с бонусом 15% к первому пополнению который будет действовать 24 часа
...
- [vdsina.ru](https://vdsina.ru/?partner=mvi4214r3b6m) - только РФ - ссылка для регистрации с бонусом 10% к первому пополнению
...
5. Если на прокси-сервере MTU меньше 1500, необходимо уменьшить в конфигурационных файлах OpenVPN (/etc/openvpn/server/\*.conf) и WireGuard (/etc/wireguard/\*.conf) значение MTU (по умолчанию 1420) на разницу между 1500 и текущим показанным значением, перезагрузить сервер
6. На сервере АнтиЗапрета в файл /root/antizapret/config/allow-ips.txt записать IPv4-адрес прокси-сервера и выполнить
```
...
### 8. Почему сайт не открывается или открывается наполовину?

AntiZapret VPN принимает решение о маршрутизации **по каждому домену отдельно**, а не по сайту целиком\
Через туннель уходят только:
- подменные IP-адреса 10.30.0.0/15 (172.30.0.0/15 или 198.18.0.0/15) - их DNS АнтиЗапрета выдает только для доменов, попавших в список (`result/include-hosts.txt` → `/etc/knot-resolver/proxy.rpz`)
- реальные IP-адреса из `result/route-ips.txt` - это `config/include-ips.txt` плюс включенные списки IP-адресов сервисов и хостингов

Все остальные домены DNS АнтиЗапрета отдает с реальными IP-адресами, и трафик к ним идет **напрямую через провайдера клиента** со всеми его блокировками\
Одна веб-страница подгружает ресурсы с десятков хостов (картинки, стили, скрипты, шрифты, API), и каждый из них маршрутизируется независимо\
Поэтому и получаются три разных результата:
- **сайт открывается** - все нужные странице домены есть в списке
- **сайт не открывается** - основного домена в списке нет, и провайдер его блокирует
- **сайт открывается наполовину** - часть доменов страницы есть в списке, а часть нет: HTML приходит, а картинки, стили или скрипты - нет

Отдельный случай - сайт заблокирован **по IP-адресу или по диапазону зарубежного хостинга**, но самого домена нет в реестре РКН\
Такой домен не попадает в список АнтиЗапрета автоматически, хотя у провайдера клиента не работает

Это не поломка, а граница возможностей: список АнтиЗапрета строится **только из доменных источников** - реестра РКН (`reestr_hostname.txt`), `antifilter.download/list/domains.lst` и `download/include-hosts.txt` этого репозитория\
Списка заблокированных IP-адресов у АнтиЗапрета нет вообще, а все файлы `download/*ips.txt` - это диапазоны конкретных сервисов и хостингов, а не перечень блокировок\
Поэтому о блокировке, сделанной по IP-адресу или по SNI без записи в реестре, АнтиЗапрет узнать не может, и такие домены нужно добавлять руками\
Пускать же через туннель весь диапазон хостинга целиком по умолчанию нельзя: например `51.80.0.0/12` у OVH - это больше миллиона адресов, и вместе с нужным сайтом через ваш сервер пойдет весь остальной трафик к этому хостингу

Что делать:
1. Проверить домены страницы (адреса всех запросов видно на вкладке Network в инструментах разработчика браузера, F12)
```
/root/antizapret/check-host.sh <домен_или_ссылка>
```
2. Добавить недостающие домены в `config/include-hosts.txt`, либо их IP-адреса в `config/include-ips.txt`, и выполнить
```
/root/antizapret/doall.sh
```
3. Если сайт лежит на зарубежном хостинге, целиком заблокированном у провайдера, можно включить в файле `/root/antizapret/setup` готовые списки IP-адресов хостингов: `AMAZON_INCLUDE`, `HETZNER_INCLUDE`, `DIGITALOCEAN_INCLUDE`, `OVH_INCLUDE`, `GOOGLE_INCLUDE`, `AKAMAI_INCLUDE`, `ROBLOX_INCLUDE`, `DISCORD_INCLUDE`, поставив им значение `y`, и выполнить `/root/antizapret/doall.sh`\
Это заметно увеличивает объем трафика через сервер, поэтому включайте только нужные списки

Добавление доменов работает сразу после `doall.sh`, а после добавления IP-адресов клиентам OpenVPN достаточно переподключиться, клиентам же WireGuard/AmneziaWG нужно обновить строку AllowedIPs в конфигурационных файлах (antizapret-\*.conf)

Если же сайт не открывается вообще ни один, а не наполовину - причина обычно другая: браузер или устройство использует свой безопасный DNS в обход DNS АнтиЗапрета, смотрите пункты 1-4 в начале этого файла
